...
class _AdminRemedyEditScreenState extends State<AdminRemedyEditScreen> {
  ...
  void _populateFromRemedy() {
    ...
    if (remedyIdForProducts != null) {
      SupabaseService.supabase.from('remedy_products')
          .select('products(*)')
          .eq('remedy_id', remedyIdForProducts)
          .then((data) {
        if (mounted) setState(() => _linkedProducts = (data as List).map((e) => (e['products'] as Map<String, dynamic>)).toList());
      });
    }
  }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _ProductPickerSheet(
        ...
        onSelect: (product) async {
          ...
          await SupabaseService.supabase.from('remedy_products')
              .upsert({'remedy_id': widget.remedy['id'], 'product_id': product['id']});
          // Also update product linked_remedy_id for backward compatibility
          await SupabaseService.supabase.from('products')
              .update({'linked_remedy_id': widget.remedy['id']}).eq('id', product['id']);
          final updated = await SupabaseService.supabase.from('remedy_products')
              .select('products(*)')
              .eq('remedy_id', widget.remedy['id']);
          setState(() => _linkedProducts = (updated as List).map((e) => (e['products'] as Map<String, dynamic>)).toList());
        },
      ),
    );
  }
  ...
  Future<void> _unlinkProduct(String productId) async {
    await SupabaseService.supabase.from('remedy_products')
        .delete().eq('remedy_id', widget.remedy['id']).eq('product_id', productId);
    ShopScreen.reload();
