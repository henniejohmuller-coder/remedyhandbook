...
class _RecipesScreenState extends State<RecipesScreen>
    ...
    try {
      ...
      setState(() {
        _allRemedies = remedies.map((r) => {
          ...r,
          '_has_component': products.any((p) => p['linked_remedy_id']?.toString() == r['id']?.toString() && p['type'] == 'Component'),
          '_has_remedy':    products.any((p) => p['linked_remedy_id']?.toString() == r['id']?.toString() && p['type'] == 'Remedy'),
        }).toList();
      });
    } catch (_) {}
  }
  ...
  void _goToProduct(BuildContext context, Map<String, dynamic> remedy, bool isHerb) async {
    try {
      final remedyId = remedy['id']?.toString() ?? '';
      final products = await SupabaseService.getProducts();
      ...
      if (linked.isNotEmpty && context.mounted) {
        Navigator.push(context, MaterialPageRoute(
          builder: (_) => ProductDetailScreenDB(product: linked.first)));
      } else if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(isHerb
              ? 'No component product linked to this remedy yet.'
              : 'No remedy product linked yet.'),
            ...
            Row(children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: hasComponent ? onBuyHerb : null,
                  style: OutlinedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                    side: BorderSide(color: hasComponent ? AppColors.dark : Colors.grey.shade300),
                  ),
                  child: Text('Buy Component',
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
                      color: hasComponent ? AppColors.dark : Colors.grey.shade400)),
                ),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: ElevatedButton(
                  onPressed: hasRemedy ? onBuyRemedy : null,
                  style: ElevatedButton.styleFrom(
                    backgroundColor: hasRemedy ? AppColors.primary : Colors.grey.shade200,
                    foregroundColor: hasRemedy ? AppColors.dark : Colors.grey.shade400,
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                    elevation: 0,
                  ),
