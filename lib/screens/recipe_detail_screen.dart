...
class _RecipeDetailScreenDBState extends State<RecipeDetailScreenDB> {
  ...
  void initState() {
    ...
    if (!widget.isGuest) _loadSavedState();
    _checkProducts();
    _loadExistingReview();
    _checkAdmin();
    _loadInstructions();
  ...
  Future<void> _checkProducts() async {
    try {
      ...
      final data = await SupabaseService.supabase
          ...
          .order('name');
      // Also get products from remedy_products junction table
      final rpData = await SupabaseService.supabase
          .from('remedy_products')
          .select('products(*)')
          .eq('remedy_id', remedyId);
      final rpProducts = (rpData as List)
          .map((e) => e['products'] as Map<String, dynamic>)
          .toList();
      final all = [...(data as List).cast<Map<String, dynamic>>()];
      for (final rp in rpProducts) {
        if (!all.any((p) => p['id'] == rp['id'])) all.add(rp);
      }
      setState(() {
  ...
  Future<void> _loadSavedState() async {
    try {
      final remedyId = _remedy['id']?.toString() ?? '';
      final saved = await SupabaseService.isRemedySaved(remedyId);
  ...
  Future<void> _toggleSave() async {
    ...
    final remedyId = _remedy['id']?.toString() ?? '';
    try {
      if (_isSaved) { await SupabaseService.unsaveRemedy(remedyId); } else { await SupabaseService.saveRemedy(remedyId); }
  ...
  void _showGuestPopup(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Sign up or log in to rate, review, and buy remedies.',
  ...
  void _shareRemedy() {
    final remedyId = _remedy['id']?.toString() ?? '';
    final name = _remedy['name'] ?? 'this remedy';
    final link = 'https://remedyhandbook.com/remedy/$remedyId';
    final message = 'Check out $name on Remedy Handbook!' + String.fromCharCode(10) + String.fromCharCode(10) + 'Get the app: https://remedy-handbook.web.app/download' + String.fromCharCode(10) + 'View this remedy: $link';
    Share.share(message);
  }
  ...
  bool _requireAuth(BuildContext context) {
    if (widget.isGuest) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: const Text('Sign up to continue', style: AppTextStyles.heading3),
          content: const Text('Sign up or log in to rate, review, and buy remedies.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel', style: TextStyle(color: AppColors.textSecondary))),
            ElevatedButton(onPressed: () { Navigator.pop(ctx); context.go('/'); }, child: const Text('Sign up / Log in')),
          ],
        ),
      );
      return false;
  ...
  Future<void> _goToProduct(BuildContext context, bool isHerb) async {
    if (!_requireAuth(context)) return;
    final primaryHerb = _remedy['primary_herb']?.toString().isNotEmpty == true ? _remedy['primary_herb']!.toString() : _remedy['component']?.toString() ?? '';
    final mainConstituent = _remedy['main_constituent']?.toString().isNotEmpty == true ? _remedy['main_constituent']!.toString() : _remedy['constituent']?.toString() ?? '';
                ...
                if (!widget.isGuest)
                  GestureDetector(
                    ...
                    child: Padding(
                      ...
                      child: Icon(_isSaved ? Icons.favorite : Icons.favorite_border, size: 22, color: _isSaved ? Colors.red : AppColors.dark),
                    ),
                  ),
                GestureDetector(onTap: _shareRemedy, child: const Padding(padding: EdgeInsets.only(right: 8), child: Icon(Icons.share_outlined, size: 22, color: AppColors.dark))),
                if (_isAdmin)
                  GestureDetector(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(child: OutlinedButton(
                        onPressed: widget.isGuest ? () => _showGuestPopup(context) : () => _goToProduct(context, true),
                        style: OutlinedButton.styleFrom(
                      ...
                      Expanded(child: ElevatedButton(
                        onPressed: widget.isGuest ? () => _showGuestPopup(context) : () => _goToProduct(context, false),
                        style: ElevatedButton.styleFrom(
                    ...
                    if (_submitted && !_isEditing)
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: widget.isGuest ? () => _showGuestPopup(context) : (_selectedRating == 0 ? null : _submitReview),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: AppColors.primary, foregroundColor: AppColors.dark,
                      ...
                      widget.isGuest ? Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    width: double.infinity,
                                    child: ElevatedButton(onPressed: () => context.go('/'), child: const Text('Sign up / Log in')),
                                  ),
                                ],
                              ),
